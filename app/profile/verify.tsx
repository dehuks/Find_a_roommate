    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        ...
        <TouchableOpacity
          ...
          disabled={loading}
          className={`w-full py-4 rounded-xl mt-8 ${loading ? 'bg-slate-300' : 'bg-blue-600'}`}
...
const DocTypeOption = ({ label, selected, onPress }: any) => (
  <TouchableOpacity
    onPress={onPress}
    className={`flex-1 py-3 rounded-xl border items-center justify-center ${selected ? 'bg-blue-50 border-blue-600' : 'bg-white border-slate-200'
      }`}
  >
