  ...
  const renderMessage = ({ item }: { item: any }) => {
    ...
    return (
      <View className={`flex-row mb-3 ${isMe ? 'justify-end' : 'justify-start'}`}>
        ...
        <View
          className={`max-w-[75%] px-4 py-3 rounded-2xl ${isMe ? 'bg-blue-600 rounded-tr-none' : 'bg-slate-100 rounded-tl-none'
            }`}
        >
      ...
      <SafeAreaView className="flex-1" edges={['bottom']}>
        ...
        >
          <View className="flex-row items-center p-3 border-t border-slate-100 bg-white">
            ...
            <TouchableOpacity
              onPress={handleSend}
              className={`w-12 h-12 rounded-full items-center justify-center ${inputText.trim() ? 'bg-blue-600' : 'bg-slate-200'
                }`}
