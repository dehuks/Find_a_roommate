...
export default function RegisterScreen() {
  ...
  const SelectionChip = ({ label, value, selectedValue, onSelect }: any) => (
    <TouchableOpacity
      ...
      className={`px-6 py-3 rounded-full border mr-2 mb-2 ${selectedValue === value
          ? 'bg-blue-600 border-blue-600'
          : 'bg-white border-slate-200'
        }`}
    >
      <Text className={`font-semibold ${selectedValue === value ? 'text-white' : 'text-slate-600'
        }`}>
        {label}
      </Text>
    </TouchableOpacity>
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      >
        ...
        <View className="space-y-4">
          ...
          <TouchableOpacity
            className="w-full bg-blue-600 h-16 rounded-full justify-center items-center mt-6 active:bg-blue-700"
